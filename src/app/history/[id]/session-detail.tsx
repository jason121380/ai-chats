...
import { Coins, FileText, Gavel } from "lucide-react"
...
export function SessionDetail({
  ...
  id: string
  /**
   * The session as the server already rendered it. Seeding state with this
   * is the point of the split: the conversation is in the HTML, so it is on
   * screen before any of this component's JavaScript has been fetched,
   * parsed or hydrated — and the first thing a reader sees is the meeting,
   * not a spinner waiting for a round trip that has not started yet.
   */
  initial: SessionDetailDto
}) {
  const { models } = useModels()
  const money = useMoney()
  const [session, setSession] = useState<SessionDetailDto>(initial)
